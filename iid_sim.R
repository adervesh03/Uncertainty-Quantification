...
##########################################################################
# function: model_fits
# takes in: data frame data, train/test split
# folds,  Bootstrap size B 
# returns: matrices containing uncertainty for each of our three models.
model_fits <- function(data, train_idx, test_idx,  B, verbose = TRUE) {
...
compute_simulation <- function(n, J, B, version, train_frac = 0.8) {
  sim_seed = 70 + J
  set.seed(sim_seed)
  if (version == "linear") {
